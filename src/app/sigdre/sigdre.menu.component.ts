...
export class AppMenuComponent implements OnInit {
    ...
    ngOnInit() {
        console.log(JSON.parse(localStorage.getItem('cargo')).cargo);
        let cargo = JSON.parse(localStorage.getItem('cargo')).cargo;
        switch (cargo) {
            case 1:
                this.model = [
                    {label: 'Dashboard', icon: 'dashboard', routerLink: ['./']},
                    {label: 'Usuarios', icon: 'accessibility', routerLink: ['./usuarios']},
                    {
...
export class AppSubMenu {
    ...
    constructor(@Inject(forwardRef(() => SigdreComponent)) public app:SigdreComponent, public router: Router, public location: Location) {}
